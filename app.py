...
# พยายามใช้ทั้งสองวิธี - ทั้ง component path และ HTML component

# 1. พยายามใช้ built React component
try:
    ...
    if os.path.isdir(build_dir):
        ...
        st.write("Using built React component")
except Exception as e:
    st.error(f"Error loading built component: {e}")
    component_value = None

# 2. หากวิธีแรกไม่สำเร็จ ให้ใช้ HTML component แบบ inline
if component_value is None:
    st.write("Falling back to HTML component")
    # HTML component
    component_html = """
    <!DOCTYPE html>
    <html lang="en">
        ...
        <script>
            ...
            window.addEventListener("message", function(event) {
                ...
                if (event.data.type === "streamlit:render") {
                    ...
                    // ทำอะไรบางอย่างกับข้อมูลที่ได้รับ (ถ้าจำเป็น)
                }
            });
    ...
    component_value = dishcovery_component

# ตัวแปร component_value จะมีค่าเมื่อมีการส่งข้อมูลกลับมาจาก component
if component_value:
...
st.markdown("---")
st.markdown("### วิธีการแก้ไขปัญหา")
st.info("""
หากแอพยังไม่แสดงผล ลองทำตามขั้นตอนเหล่านี้:

1. **ตรวจสอบโครงสร้างไฟล์**:
   - ตรวจสอบว่ามีไฟล์ครบตามที่กำหนด
   - ตรวจสอบว่าไฟล์ app.py อยู่ในตำแหน่งที่ถูกต้อง
   
2. **ตรวจสอบการ deploy**:
   - หลังจาก push โค้ดไปยัง GitHub แล้ว ให้รอสักครู่ให้ Streamlit Cloud ทำการ deploy
   - ตรวจสอบ deployment log ใน Streamlit Cloud
   
3. **ใช้ HTML Component**:
   - หากยังมีปัญหา แนะนำให้ใช้เฉพาะวิธี HTML Component แทน
   
4. **ตรวจสอบ Browser Console**:
   - ตรวจสอบ console ของเบราว์เซอร์สำหรับข้อผิดพลาด (F12)
""")